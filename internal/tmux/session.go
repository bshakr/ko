...
func CreateSession(repoName, worktreeName, worktreePath string, cfg *config.Config) error {
	...
	cmd := exec.Command("tmux", "new-window", "-n", windowName, "-c", worktreePath)
	...
	if err := runTmuxCmd("split-window", "-h", "-c", worktreePath); err != nil {
		return err
	}

	// Split left pane horizontally
	if err := runTmuxCmd("select-pane", "-t", "0"); err != nil {
		return err
	}
	if err := runTmuxCmd("split-window", "-v", "-c", worktreePath); err != nil {
		return err
	}

	// Split right pane horizontally
	if err := runTmuxCmd("select-pane", "-t", "2"); err != nil {
		return err
	}
	if err := runTmuxCmd("split-window", "-v", "-c", worktreePath); err != nil {
		return err
	...
	if err := sendKeys(2, waitCmd); err != nil {
		return err
	}

	// Pane 3 (bottom-right): from config
	if err := sendKeys(3, paneCommands[3]); err != nil {
		return err
	}
...
func sendKeys(pane int, keys string) error {
	...
	cmd := exec.Command("tmux", "send-keys", "-t", paneTarget, keys, "C-m")
	if err := cmd.Run(); err != nil {
